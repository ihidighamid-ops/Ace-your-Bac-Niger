...
router.get('/', async function(req, res) {
  try {
    const { data: user } = await supabase
      .from('users').select('*').eq('id', req.session.userId).maybeSingle();
    if (!user) { req.session.destroy(); return res.redirect('/'); }
    if (!user.actif)                                    { req.session.destroy(); return res.redirect('/connexion'); }
    if (!user.paye || user.statut !== 'valide')         { return res.redirect('/statut-paiement'); }
...
@media(max-width:850px){.sidebar{transform:translateX(-255px);}.sidebar.open{transform:translateX(0);}.main{margin-left:0!important;}}
.overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:199;}
...
/* TOPBAR */
.topbar{position:sticky;top:5px;z-index:100;background:rgba(8,13,26,1);border-bottom:1px solid rgba(255,255,255,0.06);padding:11px 16px;display:flex;align-items:center;gap:10px;}
...
/* TOOL CARD — vraie balise a, compatible iOS Safari/Chrome */
.tool-card{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:15px;cursor:pointer;display:flex;flex-direction:column;text-decoration:none;-webkit-tap-highlight-color:rgba(255,117,24,0.15);touch-action:manipulation;}
.tool-card:active{background:rgba(255,255,255,0.07);border-color:rgba(255,255,255,0.2);}
.ia-card{border-color:rgba(99,102,241,0.3)!important;background:rgba(99,102,241,0.05)!important;}
.tool-top{padding:18px 14px 14px;min-height:70px;display:flex;flex-direction:column;justify-content:flex-end;border-radius:14px 14px 0 0;}
  ...
  <div style="padding:14px 12px 8px;">
    <div style="display:flex;align-items:center;gap:8px;padding:8px 8px 14px;border-bottom:1px solid rgba(255,255,255,0.07);">
      ...
      <div>
        ...
        <div class="mono" style="font-size:9px;color:#FF7518;letter-spacing:0.05em;">2026 · v7</div>
      </div>
    </div>
